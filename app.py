...
def load_data():
    if not os.path.exists(os.path.join("c:\\DormSystem", DB_FILE)):
        return {"settings": {"deadline": "2026-12-31 23:59"}, "applications": []}
    with open(os.path.join("c:\\DormSystem", DB_FILE), "r", encoding="utf-8") as f:
        return json.load(f)

def save_data(data):
    with open(os.path.join("c:\\DormSystem", DB_FILE), "w", encoding="utf-8") as f:
...
def student_form():
    ...
    if request.method == "POST":
        ...
        # 決定這一筆申請的主日期（工讀看work_date，省親看開始日期，回國看回國日期）
        target_date = ""
        ...
        # 🎯 【超強防覆蓋邏輯】：只刪除「同一個人」在「同一天」的舊資料，其他天的保留！
        db["applications"] = [
